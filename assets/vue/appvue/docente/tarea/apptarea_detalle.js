...
var vu = new Vue({
  ...
  methods: {
    ...
    showAll() {
      ...
      }).then(response => (this.reportes = response.data.reporte));
    },
    reset() {
      this.success = false;
      this.error = false;
    ...
    showAllEstatusTarea() {
      axios.get(this.url + "Tarea/showAllEstatusTarea/").then(response => (this.estatustarea = response.data.estatustarea));
    },
    ...
    updateAlumnosTareas() {
      ...
      axios.post(this.url + "Tarea/calificarTareaAlumno", formData).then(function (response) {
        if (response.data.error) {
          ...
          swal({position: "center", type: "success", title: "Calificado!", showConfirmButton: false, timer: 1500});
    ...
    exportExcel: function () {
      let data = XLSX.utils.json_to_sheet(this.reportes, {
        header: [
          "curp",
          "apellidop",
          "apellidom",
