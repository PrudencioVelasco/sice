...
var ve = new Vue({
  ...
  data: {
    ...
    total_que_debe_pagar_inscripcion: 0.0,
    total_que_debe_pagar_reinscripcion: 0.0,
    total_que_debe_pagar_colegiatura: 0.0,
    newBuscarCiclo: {
      idalumno: my_var_2,
      idperiodo: "",
      msgerror: ""
  ...
  methods: {
        ...
        if (ve.validatCantidad(ve.array_descuento)) {
          if (ve.array_formapago == 2) {
            if (ve.array_autorizacion != "") {
              var forma_pago = {
                idformapago: ve.array_formapago,
                descuento: ve.array_descuento,
                autorizacion: ve.array_autorizacion
    ...
    agregar_forma_pago_otros() {
      ...
      ve.formValidate = false;
      ve.error = false;
      if (ve.newCobroInicio.idtipopagocol.id != "") {
        if (ve.array_formapago != "" && ve.array_descuento != "") {
          if (ve.validatCantidad(ve.array_descuento)) {
            if (ve.array_formapago == 2) {
              if (ve.array_autorizacion != "") {
                var forma_pago = {
                  idconcepto: ve.newCobroInicio.idtipopagocol.id,
                  idformapago: ve.array_formapago,
                  concepto: ve.newCobroInicio.idtipopagocol.text,
                  descuento: ve.array_descuento,
                  autorizacion: ve.array_autorizacion
                }; //creamos la variable personas, con la variable nombre y apellidos
                ve.array_pago_colegiatura.push(forma_pago); //añadimos el la variable persona al array
        ...
        ve.mostar_error_formapago = true;
        ve.error_formapago = "Seleccione el concepto.";
        ve.error_pago = false;
      }
    },
    deleteItem(index) {
    ...
    onChange(event) {
      ...
      var idconcepto = ve.newCobroInicio.idtipopagocol.id;
    ...
    estadocuentaAll() {
      ...
      axios.get(this.url + "EstadoCuenta/showAllMeses/", {
        ...
        if (response.data.meses == null) {} else {
          ve.meses = response.data.meses;
        }
      });
    },
    showAllTutoresDisponibles() {
      axios.get(this.url + "EstadoCuenta/showAllCicloEscolar/").then(response => (this.ciclos = response.data.ciclos));
    },
    showAllFormasPago() {
      axios.get(this.url + "EstadoCuenta/showAllFormasPago/").then(response => (this.formaspago = response.data.formaspago));
    },
    showAllTiposPago() {
      axios.get(this.url + "EstadoCuenta/showAllTipoPago/").then(response => (this.tipospago = response.data.tipospago));
    },
    dBuscandoAbtnbuscar() {
      ve.btnbuscar = true;
      ve.loaging_buscar = false;
    ...
    showAllMeses() {
      axios.get(this.url + "EstadoCuenta/showAllMeses/").then(response => (this.meses = response.data.meses));
    },
