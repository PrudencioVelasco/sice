...
class pGrupo extends CI_Controller
{
	...
	public function index()
	{
		...
		$result = $this->grupo->showAllGruposProfesor($this->session->idprofesor, $this->session->idplantel);
	...
	public function calificacionLic($idhorario = '', $idhorariodetalle)
	{
		...
		if ((isset($idhorario) && !empty($idhorario)) && (isset($idhorariodetalle) && !empty($idhorariodetalle))) {
			...
			$data = array(
				...
				'unidades_trabajo_final' => $unidades_trabajo_final,
      ...
      <th>NOMBRE</th>';
		...
		if (isset($alumnos) && !empty($alumnos)) {
			foreach ($alumnos as $row) {
				...
				if (isset($unidades) && !empty($unidades)) {
					foreach ($unidades as $block) :
						...
						if ($val != false) {
							...
							$tabla .= '<label>' . round($val->calificacion, 0) . '  </label>';
							$fecha_inicio = date('Y-m-d');
							$fecha_fin = date('Y-m-d', strtotime($val->fecharegistro));
							$total_dias = dias_pasados($fecha_inicio, $fecha_fin);
							if ($total_dias <= $this->dias) {
				...
				if ((isset($unidades_materia) && !empty($unidades_materia) && $unidades_materia > 0) && (isset($suma_calificacion) && !empty($suma_calificacion) && $suma_calificacion > 0)) {

					$promedio_unidades = numberFormatPrecision($suma_calificacion / $unidades_materia, 1, '.');
				}
				if ($total_unidades_registradas == $unidades_materia) {
				...
				if ((isset($total_unidades_trabajo_final) && !empty($total_unidades_trabajo_final) && $total_unidades_trabajo_final > 0) && (isset($suma_calificacion_trabajo_final) && !empty($suma_calificacion_trabajo_final) && $suma_calificacion_trabajo_final > 0)) {
					$promedio_trabajo_final = numberFormatPrecision($suma_calificacion_trabajo_final / $total_unidades_trabajo_final, 1, '.');
				}
				if (($total_unidades_registradas == $unidades_materia) && ($total_unidades_trabajo_final == $total_trabajo_final_registradas)) {
		...
		$config = array(
			array(
				...
				'errors' => array(
					...
					'decimal' => 'Debe de ser solo número decimal la calificacion.'
				)
			)
		);
		$this->form_validation->set_rules($config);
