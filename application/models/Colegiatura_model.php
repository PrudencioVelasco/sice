...
class Colegiatura_model extends CI_Model
{
    ...
    {
        $this->db->select('c.idcolegiatura, c.descuento,p.nombreplantel, c.idnivelestudio, n.nombrenivel, c.activo, c.idtipopagocol, tp.concepto, n.numeroordinaria');
    ...
    public function showAllConfiguraciones($idplantel = '')
    {
        $this->db->select('c.idconfiguracion,c.diaultimorecargo,c.totalrecargo');
    ...
    public function searchConcepto($match)
    {
        $field = array(
            'c.concepto',
        );
        $this->db->select('c.*');
        $this->db->from('tbltipopagocol c');
        $this->db->like('concat(' . implode(',', $field) . ')', $match);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result();
    ...
    {
        $this->db->insert('tbltipopagocol', $data);
        $insert_id = $this->db->insert_id();
        return  $insert_id;
    }
    ...
    public function updateConcepto($idconcepto = '', $field)
    {
        $this->db->where('idtipopagocol', $idconcepto);
        $this->db->update('tbltipopagocol', $field);
        if ($this->db->affected_rows() > 0) {
            return true;
        } else {
            return false;
    ...
    public function updateRecargo($idconfiguracion = '', $field)
    {
    ...
    public function deleteRecargo($idconfiguracion)
    {
        # code...
        $this->db->where('idconfiguracion', $idconfiguracion);
        $this->db->delete('tblconfiguracion');
        if ($this->db->affected_rows() > 0) {
            return true;
        } else {
            return false;
